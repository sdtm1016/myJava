...
public class PairingHeap<AnyType extends Comparable<? super AnyType>> {
	...
	public void decreaseKey(Position<AnyType> pos, AnyType newVal) {
		if (pos == null)
			throw new IllegalArgumentException(
					"null Position passed to decreaseKey");
		...
		if (p.element.compareTo(newVal) < 0)
			throw new IllegalArgumentException("newVal/oldval: " + newVal
					+ " /" + p.element);
		p.element = newVal;
		if (p != root) {
	...
	private PairNode<AnyType>[] doubleIfFull(PairNode<AnyType>[] array,
			int index) {
